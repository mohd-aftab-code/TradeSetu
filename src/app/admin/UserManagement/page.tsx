...
const UserManagement: React.FC = () => {
  ...
  const filteredUsers = users.filter(user => {
    ...
    return matchesSearch && matchesPlan && matchesStatus && matchesRole && matchesTrades;
  });
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} className="border-t border-white/10 hover:bg-white/5">
                  ...
                  <td className="p-4">
                    <p className="text-purple-200 text-sm">{new Date(user.last_login).toLocaleDateString()}</p>
                  </td>
                  <td className="p-4">
